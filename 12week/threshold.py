...
def get_threshold(src, type):
    ...
    if type == 'rice':
        ...
        # 여러줄로 작성하셔도 상관 없습니다.
        sum_hist = sum(hist) - hist[0]
        hist[0] = 0
        p = hist / sum(hist)
    print(hist.astype(np.uint8))
    k_opt_warw = []
    k_opt_warb = []
    for k in range(256):
        # 각각 한 줄로 작성하세요
        ...
        m1 = sum(p[:k+1] * intensity[:k+1]) / q1
        m2 = sum(p[k+1:] * intensity[k+1:]) / q2

        mg = q1*m1 + q2*m2

        var1 = sum(intensity[:k+1]**2*p[:k+1]) / q1  - m1**2
        var2 = sum(intensity[k+1:]**2*p[k+1:]) / q2  - m2**2
    ...
    print(k_opt_warw.argmin(), k_opt_warb.argmax())
...
def rice_main():
    ...
    print('< cv2.threshold >')
    print(val)
    dst, threshold_value = get_threshold(src, 'rice')
    print('< get_threshold >')
...
def meat_main():
    ...
    final = meat.astype(np.uint16) + dst.astype(np.uint16)
